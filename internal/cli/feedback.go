...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".hyperliquid-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HYPERLIQUID_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HYPERLIQUID_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hyperliquid-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.hyperliquid-pp-cli/feedback.jsonl.
When ` + "`HYPERLIQUID_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HYPERLIQUID_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hyperliquid feedback list
  hyperliquid feedback list --limit 5
  hyperliquid feedback list --json`,
