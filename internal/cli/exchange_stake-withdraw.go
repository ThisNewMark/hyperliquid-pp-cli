// Hand-rewritten: typed cWithdraw + signed user-signed POST.
...
func newExchangeStakeWithdrawCmd(flags *rootFlags) *cobra.Command {
	var wei int64

	cmd := &cobra.Command{
		Use:   "stake-withdraw",
		Short: "Begin a 7-day unstake from the staking layer (signed user action — MAIN wallet)",
		Example: `  hyperliquid exchange stake-withdraw --wei 100000000`,
		Annotations: map[string]string{
			"pp:endpoint": "exchange.stake-withdraw",
			"pp:method":   "POST",
			"pp:path": "/exchange/stakeWithdraw", "mcp:hidden": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			primary, payloadTypes := sign.CWithdrawSpec()
	...
	cmd.Flags().Int64Var(&wei, "wei", 0, "Amount of HYPE to unstake in wei (required)")
	return cmd
}
