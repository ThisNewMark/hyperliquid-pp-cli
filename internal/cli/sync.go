...
Exit codes & warnings:
  ...
  hyperliquid sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  hyperliquid sync --full

  # Incremental sync: only records from the last 7 days
  hyperliquid sync --since 7d

  # Parallel sync with 8 workers
  hyperliquid sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  hyperliquid sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/hyperliquid-pp-cli/data.db)")
